...
async function writeCodexPluginBundle(): Promise<void> {
  await rm(CODEX_PLUGIN_DIR, { recursive: true, force: true });
  await mkdir(CODEX_PLUGIN_DIR, { recursive: true });

  await Promise.all([
    writeJson(
      join(CODEX_PLUGIN_DIR, ".claude-plugin", "plugin.json"),
      getClaudePluginManifest(),
    ),
    writeJson(
      join(CODEX_PLUGIN_DIR, ".claude-plugin", "marketplace.json"),
      getClaudePluginMarketplaceManifest(),
    ),
    writeJson(
    ...
    copyDirectory(
      ...
      join(CODEX_PLUGIN_DIR, "instinct-packs"),
    ),
    copyDirectory(
      join(REPO_ROOT, "templates", "planning-with-files"),
      join(CODEX_PLUGIN_DIR, "templates", "planning-with-files"),
    ),
    ...
    copyFileTo(
      ...
      join(CODEX_PLUGIN_DIR, "lib", "plugin-metadata.mjs"),
    ),
  ...
  await writeJson(
    join(CODEX_PLUGIN_DIR, "hooks", "hooks.json"),
    getCodexPluginHooksConfig(),
  );
